...
#VerticalPodAutoscalerSpec: {
	// Recommender responsible for generating recommendation for this
	// object.
	// List should be empty (then the default recommender will
	// generate the
	...
	resourcePolicy?: {
		...
		containerPolicies?: [...{
			...
			// If not specified, the default of [ResourceCPU, ResourceMemory]
			// will be used.
	...
	// If not specified, all fields in the `PodUpdatePolicy` are set
	// to their
	// default values.
	updatePolicy?: {
		// EvictionRequirements is a list of EvictionRequirements that
		// need to
		// evaluate to true in order for a Pod to be evicted. If more than
		// one
		// EvictionRequirement is specified, all of them need to be
		// fulfilled to allow eviction.
		evictionRequirements?: [...{
			// EvictionChangeRequirement refers to the relationship between
			// the new target recommendation for a Pod and its current
			// requests, what kind of change is necessary for the Pod to be
			// evicted
			changeRequirement!: "TargetHigherThanRequests" | "TargetLowerThanRequests"

			// Resources is a list of one or more resources that the condition
			// applies
			// to. If more than one resource is given, the EvictionRequirement
			// is fulfilled
			// if at least one resource meets `changeRequirement`.
			resources!: [...string]
		}]

		// Minimal number of replicas which need to be alive for Updater
		// to attempt
		// pod eviction (pending other checks like PDB). Only positive
		// values are
